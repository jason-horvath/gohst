...
func initSession() {
	adminName := GetEnv("SESSION_ADMIN_NAME", "").(string)
	if adminName == "" {
		adminName = GetEnv("ADMIN_SESSION_NAME", "_gohst_admin_session").(string)
	}
	Session = &SessionConfig{
		...
		Name:      GetEnv("SESSION_NAME", "_gohst_session").(string),
		AdminName: adminName,
		Redis: &RedisConfig{
